...
module puzzle2 =
    ...
    let is_safe2 (line: int list) =
        ...
        | _ ->
            let isIncreasing =
                List.pairwise line |> List.forall (fun (x, y) -> x < y && y - x < 4)

            let isDecreasing =
                List.pairwise line |> List.forall (fun (x, y) -> x > y && x - y < 4)
        ...
        | _ ->
            if isIncreasing line || isDecreasing line then
                ...
                let checkWithRemoval =
                    line
                    |> List.mapi (fun i _ -> line |> filteri (fun j _ -> j <> i) // Remove one element
                    )
    ...
    let result (line_arr: (int * int * int * int * int) array) =
        line_arr |> Array.filter is_safe |> Array.length
